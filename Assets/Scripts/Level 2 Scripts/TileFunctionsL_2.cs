...
public class TileFunctionsL_2 : MonoBehaviour
{
	...
	private void PurchasedClick()
    {
        ...
        if ((!tile.Purchased && !tile.Locked) && tile.Cost <= GameTiles.instance.budget)
        {
			...
			tile.Purchased = true;
			GameTiles.instance.score += (tile.ecoVal + tile.bonus);
			tile.lastTotal = (tile.ecoVal + tile.bonus);
			GameTiles.instance.budget -= tile.Cost;
			GameTiles.instance.boughtTiles.Add(tile.LocalPlace, tile);
			purchaseButtonText.text = "Sell";
			for(int i = -1 ; i < 2 ; i++){
				for(int j = -1 ; j < 2 ; j++){
					if(cPlace.x+j > -6 && cPlace.x+j < 4 && cPlace.y+i > -5 && cPlace.y+i < 5){
						...
						if(cPlace.y%2 == 0){
							if(!(i==1 && j == 1) && !(i==-1 &&j==1) && !(i == 0 && j == 0)){
								...
								if(i == -1 && j == -1 && edTile.Purchased == false){
									...
									edTile.bonus += (int)(edTile.ecoVal * 0.1);
								}
	...
	private void ResetClick(){
		...
		GameTiles.instance.budget = 30000;
