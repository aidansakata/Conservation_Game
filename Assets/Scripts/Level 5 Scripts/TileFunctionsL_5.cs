...
public class TileFunctionsL_5 : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (Input.GetMouseButtonDown(0))
		{
			...
			if (tiles.TryGetValue(worldPoint, out _tile))
			{
				...
				ecoVal3Text.text = _tile.ecoVal3.ToString();
	...
	private void PurchasedClick()
    {
        ...
        if ((!tile.Purchased && !tile.Locked) && tile.Cost <= GameTiles.instance.budget)
        {
			...
			tile.Purchased = true;
			GameTiles.instance.score += (tile.ecoVal + tile.ecoVal2 + tile.ecoVal3 + tile.bonus);
			tile.lastTotal = (tile.ecoVal + tile.ecoVal2 + tile.ecoVal3 + tile.bonus);
						...
						if(cPlace.y%2 == 0){
							if(!(i==1 && j == 1) && !(i==-1 &&j==1) && !(i == 0 && j == 0)){
								...
								if(i == -1 && j == -1 && edTile.Purchased == false){
									...
									edTile.bonus += (int)((edTile.ecoVal + edTile.ecoVal2 + edTile.ecoVal3) * 0.1);
								}
	...
	private void ResetClick(){
		...
		GameTiles.instance.budget = 80000;
