...
public static class PopulateDemoLevelsOnGridManager
{
    ...
    public static void Populate()
    {
        ...
        void Fill(string fieldName, int w, int h, System.Func<int, int, int, int, int> rule)
        {
            ...
            def.EnsureSize(w, h, defaultTileType: 1, defaultCost: 1);
            def.Fill(rule);
            f.SetValue(gm, def);
        }

        Fill("level1Easy8x8", 8, 8, (x, y, w, h) => ((x + y) % 2 == 0) ? 1 : 2);
        Fill("level2Med10x10", 10, 10, (x, y, w, h) => (x == w / 2) ? 2 : 1);
        Fill("level3Hard12x12", 12, 12, (x, y, w, h) => (x == y || x == (w - 1 - y)) ? 3 : 1);
